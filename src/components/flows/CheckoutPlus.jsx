...
const CheckoutPlus = () => {
  ...
  const launchCheckoutPlus = () => {
    ...
    // Build payment options for Bolt
    ...
    if (formData.lastname) paymentOptions.lastname = formData.lastname
    if (formData.address1) paymentOptions.address1 = formData.address1
    if (formData.address2) paymentOptions.address2 = formData.address2
    if (formData.city) paymentOptions.city = formData.city
    if (formData.state) paymentOptions.state = formData.state
    if (formData.country) paymentOptions.country = formData.country
    if (formData.zipcode) paymentOptions.zipcode = formData.zipcode
    if (formData.udf1) paymentOptions.udf1 = formData.udf1
    if (formData.udf2) paymentOptions.udf2 = formData.udf2
    if (formData.udf3) paymentOptions.udf3 = formData.udf3
    if (formData.udf4) paymentOptions.udf4 = formData.udf4
    if (formData.udf5) paymentOptions.udf5 = formData.udf5
    ...
    if (selectedMethods.length > 0) {
      paymentOptions.enforce_paymethod = selectedMethods.join('|')
    }

    // Launch PayU Bolt
    if (window.PayU && window.PayU.bolt) {
      window.PayU.bolt.launch(paymentOptions, {
      ...
      alert('PayU Bolt SDK not loaded. Please check if the script is included in index.html')
    }
  }
  ...
  return (
    <>
      ...
      <div className="main-content">
          ...
          <div className="form-group">
            ...
            <div className="payment-methods">
              <div className="payment-method">
                <input type="checkbox" id="cp_cc" name="paymentMethod_cc" checked={formData.paymentMethods.cc} onChange={handleChange} />
                <label htmlFor="cp_cc">Credit Cards</label>
              </div>
              <div className="payment-method">
                <input type="checkbox" id="cp_dc" name="paymentMethod_dc" checked={formData.paymentMethods.dc} onChange={handleChange} />
                <label htmlFor="cp_dc">Debit Cards</label>
              </div>
              <div className="payment-method">
                <input type="checkbox" id="cp_nb" name="paymentMethod_nb" checked={formData.paymentMethods.nb} onChange={handleChange} />
                <label htmlFor="cp_nb">Net Banking</label>
              </div>
              <div className="payment-method">
                <input type="checkbox" id="cp_upi" name="paymentMethod_upi" checked={formData.paymentMethods.upi} onChange={handleChange} />
                <label htmlFor="cp_upi">UPI</label>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="section">
          <h2 className="section-title">Callback URLs</h2>
          <div className="form-row">
            <div className="form-group">
        ...
        <div className="pay-button-wrapper">
          ...
          <button type="button" className="button" onClick={launchCheckoutPlus}>Pay with Checkout Plus</button>
        </div>
